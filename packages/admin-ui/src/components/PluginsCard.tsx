...
export function PluginsCard({ trigger, pushLog }: Props) {
  ...
  return (
    ...
    >
      {entries.length === 0 ? (
        <EmptyState>No plugins loaded. Place plugins in the folder and refresh.</EmptyState>
      ) : (
        <ul className="max-h-72 space-y-2 overflow-y-auto scrollbar-thin pr-1">
          {entries.map(([name, p]) => (
            <li
              key={name}
              ...
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 font-mono text-[10px] text-slate-600">
                    v{p.version} · BizHawk {p.bizhawk_version}
                  </p>
                </div>
                <ActionRow>
                  <Button
